  ...
  if (!data) {
    return (
      <div className="card p-6">
        <h2 className="font-display font-bold text-sm text-slate-900 mb-2">Interview Readiness</h2>
        <p className="text-sm text-slate-500">Couldn&apos;t load readiness score right now.</p>
      </div>
    );
  }
  ...
  return (
    <div className="card p-6">
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-2 mb-5">
        <div>
          <h2 className="font-display font-bold text-base text-slate-900">Interview Readiness</h2>
      ...
      <p className="text-xs text-slate-500 mb-4">{data.insight}</p>

      <div className="grid grid-cols-1 lg:grid-cols-5 gap-5">
        <div className="lg:col-span-3 bg-slate-100 border border-slate-200 rounded-xl p-3">
        ...
        <div className="lg:col-span-2 bg-slate-100 rounded-xl p-3 border border-slate-200">
          ...
          <div className="max-h-[240px] overflow-y-auto pr-1 flex flex-col gap-2">
            {data.weakAreas.map((area) => (
              <div key={area.label} className="bg-white rounded-lg border border-slate-200 p-2.5">
                <p className="text-xs text-slate-900">{area.label} ({area.score}%)</p>
                <p className="text-[11px] text-slate-500 mt-1">{area.action}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="mt-4 bg-primary/5 border border-primary/20 rounded-xl p-3">
        ...
        {data.weakAreas[0] ? (
          <p className="text-sm text-slate-800">{data.weakAreas[0].action}</p>
        ) : (
          <p className="text-sm text-slate-800">Maintain your routine and review one mock this week.</p>
        )}
      </div>
    </div>
  );
