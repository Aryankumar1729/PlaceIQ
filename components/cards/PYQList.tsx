    ...
    <div>
      ...
      <div className="mb-4">
        <div className="flex items-center justify-between mb-2">
          ...
          <span className="text-xs text-muted">{pyqs.length} of {total} questions</span>
        </div>

        {/* Progress bar — only show when prep target exists */}
        {hasTarget && total > 0 && (
          <div className="card p-3 mb-4 border-accent/20">
            <div className="flex items-center justify-between mb-2">
              <p className="text-xs font-medium">
                🎯 Prep Progress — <span className="text-accent">{donePyqIds.size}/{total} done</span>
              </p>
            ...
            <div className="h-1.5 bg-surface2 rounded-full overflow-hidden">
              <div
                ...
                style={{
                  width: `${pct}%`,
                  background: pct === 100 ? "var(--accent-green)" : pct > 50 ? "var(--accent)" : "var(--accent-pink)",
                }}
              />
            </div>
          </div>
          ...
          <div className="flex flex-col gap-3 stagger">
            {pyqs.map((q) => {
              ...
              return (
                <div
                  key={q.id}
                  className={`card p-4 hover:border-border-2 animate-fade-up transition-all ${
                    isDone ? "opacity-60 border-accent-green/20" : ""
                  }`}
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-3 mb-3">
                        <p className={`text-sm font-medium leading-relaxed ${isDone ? "line-through text-muted" : ""}`}>
                          {q.question}
                        </p>
